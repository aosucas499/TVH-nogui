...
# Variable global para la instancia de OmxplayerWrapper
reproductor = None

def reproducir_canal(nombre_canal, config):
    global reproductor  # Accede a la variable global

    # Detener la reproducción si ya hay un reproductor en ejecución
    ...
    # Crear o reutilizar la instancia de OmxplayerWrapper
    if reproductor is None:
        reproductor = OMXPlayer(url, dbus_name='org.mpris.MediaPlayer2.omxplayer1')
    else:
        reproductor.load(url)

    # Imprimir mensajes de depuración
    if debug:
        print(f"Debug - Reproduciendo el canal: {nombre_canal}")
